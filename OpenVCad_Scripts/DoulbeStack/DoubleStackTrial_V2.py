...
#-- Stack Settings --
fluidPercent = 0.725
bellowHousing_VeroPercent = .9
numStacks = 6
includeBaffles = True
num_bellowsToPrint = 1

#-- Housing Settings --
housingVeroPercent = .95
...
#-----------------------------------------------------
#-- Housing Assembly --
housing_union = pv.Union()
housingAssembly = pv.Mesh(
    STL_Location + "/DoubleStack_Assembly - housing-1.STL",red,True
)
housing_union.add_child(housingAssembly)

topCapLeft_mesh = pv.Mesh(
    STL_Location + "/DoubleStack_Assembly - cap_top-1.STL"
)
topCapRight_mesh = pv.Translate(0,stackOffset,0,topCapLeft_mesh)
housing_union.add_child(topCapLeft_mesh)
housing_union.add_child(topCapRight_mesh)

legLeft_mesh = pv.Mesh(
    STL_Location + "/DoubleStack_Assembly - leg-1.STL"
)
legRight_mesh = pv.Translate(0,stackOffset,0,legLeft_mesh)
housing_union.add_child(legLeft_mesh)
housing_union.add_child(legRight_mesh)

housingAssembly_FGrade = pv.FGrade([str(housingVeroPercent),str(1-housingVeroPercent)], [blue,red], True)
housingAssembly_FGrade.set_child(housing_union)
...
bellows_fGrade = pv.FGrade(
    [str(bellowHousing_VeroPercent),str(1-bellowHousing_VeroPercent)], [blue,red], True
)
